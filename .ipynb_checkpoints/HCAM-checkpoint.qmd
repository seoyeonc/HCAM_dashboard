...
camimg = torch.einsum('ij,jkl -> ikl', net2[2].weight, net1(x_cat_r,).squeeze())
...
x1=x_cat_r,.squeeze().to('cpu')*Y1-torch.min(x_cat_r,.squeeze().to('cpu'))*Y1

X12=np.array(A2.to("cpu").detach(),dtype=np.float32)
Y12=torch.Tensor(cv2.resize(X12,(512,512),interpolation=cv2.INTER_LINEAR))
x12=x_cat_r,.squeeze().to('cpu')*Y12-torch.min(x_cat_r,.squeeze().to('cpu'))*Y12
...
camimg = torch.einsum('ij,jkl -> ikl', net2[2].weight, net1(x_dog_r).squeeze())
ebayesthresh = importr('EbayesThresh').ebayesthresh
...
X1=np.array(A1.to("cpu").detach(),dtype=np.float32)
Y1=torch.Tensor(cv2.resize(X1,(512,512),interpolation=cv2.INTER_LINEAR))
x1=x_dog_r.squeeze().to('cpu')*Y1-torch.min(x_dog_r.squeeze().to('cpu'))*Y1

X12=np.array(A2.to("cpu").detach(),dtype=np.float32)
Y12=torch.Tensor(cv2.resize(X12,(512,512),interpolation=cv2.INTER_LINEAR))
x12=x_dog_r.squeeze().to('cpu')*Y12-torch.min(x_dog_r.squeeze().to('cpu'))*Y12
